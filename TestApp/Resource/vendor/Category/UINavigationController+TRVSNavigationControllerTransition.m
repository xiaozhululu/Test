...
- (void)popViewControllerWithNavigationControllerTransition{
    ...
    int tabbarHeight = 0;
    id viewControllers = self.viewControllers;
    if(viewControllers && [viewControllers count] == 1)
    {
        tabbarHeight = -50;
    }
    kTRVSNextLayer = [self _layerSnapshotWithTransform:CATransform3DIdentity];
    kTRVSNextLayer.frame = (CGRect){{-CGRectGetWidth(self.view.bounds),CGRectGetMinY(self.view.bounds)}, CGSizeMake(self.view.bounds.size.width, self.view.bounds.size.height+tabbarHeight)};
...
- (void)popViewControllerWithNavigationController{
    kTRVSCurrentLayer = [self _layerSnapshotWithTransform:CATransform3DIdentity];
    
    [self popViewControllerAnimated:NO];

    kTRVSNextLayer = [self _layerSnapshotWithTransform:CATransform3DIdentity];
    kTRVSNextLayer.frame = (CGRect){{-CGRectGetWidth(self.view.bounds),CGRectGetMinY(self.view.bounds)}, CGSizeMake(self.view.bounds.size.width, self.view.bounds.size.height)};
